...
{
    public class RadioTokenHelper : ITokenHelper
    {
        private readonly DisplayableToken _displayableToken;
        private readonly string _regex = @"^[a-zA-Z0-9\s]+\|[a-zA-Z0-9\s]+$";
        private const string ValidationMessage = "FormRadio must have a name (alphanumeric|spaces) and a value (alphanumeric|spaces) specified. Example:\"name|value\"";
